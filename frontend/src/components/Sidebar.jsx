...
export default function Sidebar({ activeTab, setActiveTab }) {
  ...
  return (
    <aside className="w-64 bg-[#10233f] text-slate-300 flex flex-col justify-between shrink-0 border-r border-[#1b3859] min-h-screen">
      <div>
        ...
        <nav className="p-4 space-y-1">
          <div className="px-3 py-2 text-[10px] uppercase font-bold text-slate-500 tracking-[0.16em]">
            Workspace
          </div>
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center justify-between px-3.5 py-3 rounded-xl text-xs font-semibold transition-all ${
                  isActive
                    ? 'bg-teal-500 text-[#10233f] shadow-lg shadow-teal-950/30'
      ...
      <div className="p-4 m-4 rounded-2xl bg-white/5 border border-white/10 space-y-2">
        <div className="flex items-center gap-2 text-teal-300 text-[10px] font-bold uppercase tracking-wider">
          <Sparkles className="w-3.5 h-3.5" /> Prototype mode
        </div>
        <p className="text-[10px] text-slate-400 leading-relaxed">
          Synthetic predictions for demonstration and hackathon validation.
        </p>
